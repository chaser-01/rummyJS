...
export class Game {
     ...
     */
    constructor(playerIds, options = {}, gameId = '') {
        /// Properties ///
    ...
    */
    initializeOptions(options) {
        ...
        if (this._useJoker && this._useWildcard)
    ...
    initializeDeckJokerAndValidationCards() {
        ...
        if (this._useJoker)
            _jokerNumber = 'Joker';
        else if (this._useWildcard)
            _jokerNumber = this.getWildcardNumber(_deck);
        else
            _jokerNumber = false;
     ...
     */
    getWildcardNumber(_deck) {
        if (this._useWildcard) {
            return _deck.numbers[this._currentRound + 1 % Object.keys(_deck.numbers).length];
        }
        else
            return false;
    }
    ...
    validateGameState() {
        if (!this.checkGameEnded() || !this.checkRoundEnded())
            return false;
        ...
        for (const player of this._players) {
            ...
            for (const meld of player.melds) {
                if (meld.cards && meld.checkAndSortMeld())
    ...
    validateGameStatus(intended_GameStatus) {
        if (intended_GameStatus !== this._gameStatus)
            return false;
        return true;
    }
    /**
    ...
    checkRoundEnded() {
        if (this._players[this._currentPlayerIndex].hand.length == 0 && this._players[this._currentPlayerIndex].playing) {
            ...
            this._logger.logGameAction('checkRoundEnded', undefined, undefined, `Current player ${this._players[this._currentPlayerIndex].id} has finished hand. Ending round`);
            return false;
        }
        return true;
    }
     ...
     */
    nextRound() {
        if (!this.validateGameState() || !this.validateGameStatus(this.GameStatus.ROUND_ENDED))
            return false;
        //calculate the round _score
        if (this._currentRound != 0)
            this._score.evaluateRoundScore();
        //increment round
        this._currentRound++;
        ...
        for (const [index, player] of this._players.entries()) {
            if (!player.playing)
                this._quitPlayers.push(...this._players.splice(index, 1));
        }
        for (const [index, player] of this._quitPlayers.entries()) {
            if (player.playing)
                this._players.push(...this._quitPlayers.splice(index, 1));
        }
    ...
    nextPlayer() {
        if (!this.validateGameState() || !this.validateGameStatus(this.GameStatus.PLAYER_TURN_ENDED))
            return false;
    ...
    sortHandBySuit(playerIndex = this._currentPlayerIndex) {
        ...
        this._players[playerIndex].hand.sort((a, b) => {
            if (a.number == this._jokerNumber && b.number == this._jokerNumber)
                return Card.compareCardsSuitFirst(a, b);
            if (a.number == this._jokerNumber)
                return 1;
            if (b.number == this._jokerNumber)
                return -1;
            return Card.compareCardsSuitFirst(a, b);
        });
    ...
    drawFromDeck() {
        if (!this.validateGameState() || !this.validateGameStatus(this.GameStatus.PLAYER_TO_DRAW))
            return false;
    ...
    drawFromDiscardPile() {
        ...
        //else check if discardPile size < than draw amount. if so, return false; else draw the cards
            ...
            if (this._deck.getDiscardPileSize() < this._cardsToDrawDiscardPile)
                return false;
    ...
    createMeld(indexArr) {
        if (!this.validateGameState() || !this.validateGameStatus(this.GameStatus.PLAYER_TURN))
            return false;
        ...
        for (const index of uniqueIndexArr) {
            if (index < this._players[this._currentPlayerIndex].hand.length) {
                ...
                meldCards.push(playerHand[index]);
        }
    ...
    addToMeld(addingCardIndex, meldOwnerIndex, meldIndex) {
        ...
        if (potentialMeld.addCard(addingCard)) {
            ...
            this._logger.logWarning('addToMeld', this._players[this._currentPlayerIndex].id, { addingCardIndex, meldOwnerIndex, meldIndex }, 'Invalid meld addition');
            return false;
        }
    }
    ...
    replaceMeldCard(replacingCardIndex, meldOwnerIndex, meldIndex, replacedCardIndex) {
        ...
        if (replacedCard) {
            ...
            this._logger.logGameAction('replaceMeldCard', this._players[this._currentPlayerIndex].id, { replacingCardIndex, meldOwnerIndex, meldIndex, replacedCardIndex });
            return true;
        }
        else {
            this._logger.logWarning('replaceMeldCard', this._players[this._currentPlayerIndex].id, { replacingCardIndex, meldOwnerIndex, meldIndex, replacedCardIndex }, 'Invalid card replacement');
            return false;
        }
    }
    ...
    getGameInfoForPlayer(playerIndex = this._currentPlayerIndex) {
        let gameInfo = {
            ...
            tableMelds: this._players.reduce((acc, player) => {
