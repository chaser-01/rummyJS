...
export function validateAndSortMeld(cards, jokerNumber = false, maxSetSize = 4) {
 ...
 */
function checkForAndSortAsSequence(cards, jokerNumber = false) {
 ...
 */
function checkForAndSortAsSet(cards, jokerNumber = false, maxSetSize = 4) {
    if (cards.length < 3 || cards.length > maxSetSize)
        return false;
    //filter out jokers and sort the cards
    let jokers = [];
    let jokerlessCards = cards;
    if (jokerNumber != false)
        [jokerlessCards, jokers] = filterJokers(cards, jokerNumber);
