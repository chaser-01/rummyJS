...
export function validateMeldInGivenOrder(cards: Card[], jokerNumber: (keyof typeof numbers|false)=false, maxSetSize: number=4){
...
function checkForSequence(cards: Card[], jokerNumber: (keyof typeof numbers|false)=false){
    let isValid = true;
    
    //if the meld starts with jokers, we must start loop at the first actual card
    let startIndex = 1;
    while (cards[startIndex-1].number==jokerNumber && cards[startIndex].number==jokerNumber) startIndex++;
    ...
    for (let i=startIndex; i<cards.length; i++){
        ...
        //or if the joker chain is valid
            ...
            let prevCardValue: number;

            //if prev. card is joker, see if its a joker chain OR the meld just starts with joker(s)
            if (cards[i-1].number == jokerNumber) {
                if (curFirstCardInJokerChain!=currentCardValue) prevCardValue = curFirstCardInJokerChain;
                else continue;
            }
            else prevCardValue = cards[i-1].cardValueSuitFirst();
...
function checkForSet(cards: Card[], jokerNumber: (keyof typeof numbers|false)=false, maxSetSize=4){
    if (cards.length>maxSetSize) return false;

    //just loop through and check for all same card numbers OR joker numbers
    let cardNo = cards[0].number;
    for (let i=1; i<cards.length; i++){
        if (cards[i].number!=cardNo || cards[i].number!=jokerNumber) return false;
    }
